...
const { loading, tableData, loadTableData, handleRemove, handleBatchRemove, spinning } = useWorkflow({ github, formState, onFormStateValidator })
...
<template>
  <a-spin :spinning="spinning.loading">
    <template #tip>
      <a-progress type="circle" :percent="spinning.percent" />
    </template>
    <a-card title="工作流列表" justify="center">
      ...
      <a-table :data-source="tableData" :loading="loading" row-key="id" :scroll="{ x: 980 }">
        ...
        <a-table-column title="工作流名称" :min-width="200">
          <template #="{ record }">
            {{ record.name }}#{{ record.run_number }}
          </template>
        </a-table-column>
